...
```{r myversion, include=FALSE}
read_chunk("src/A01_GEO_query.R")
read_chunk("src/A02_download_suppfiles.R")
```
...
We excluded from downloading supplemental files which contained pattern: `r paste(paste0("'", out_strings[1:4],"' ", collapse=","), "in the file name and we also excluded flies with following pattern ", 
paste0("'", out_strings[-c(1:4)],"' ", collapse=","))` in the file extension.
Flat tables were imported into R using fread command from package data.table `r myversion("data.table")` [@Dowle2016].
MS Excel files were imported using read_excel command from package readxl `r myversion("readxl")` [@Wickham2016].
