...
RAW.tar files are included in `r supp_raw_perc` (`r n_raw`) of GEO series, whereas only RAW.tar and filelist.txt is present in `r supp_rawtaronly_perc` (`r nrow(suppfiles_rawtar_only)`) GEO series (Fig. \@ref(fig:commonfilenames)).
...
(ref:block) Retention of GEO series and supplementary files in filter for full sets of p values. Arrows from left to right, filter for human or mouse GEO sets, GEO sets with supplementary files, supplementary files with flat table formats that were downloaded, tables with p values. Number of p value sets is the number of sets with `r glue(">{nrowthreshold}")` features.
